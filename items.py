...
def place_item(request: PlaceRequestModel):
    # Add debug logging
    logger.debug(f"Looking for container ID: '{request.containerId}'")
    logger.debug(f"Available containers: {[c.id for c in containers.values()]}")
    
    # Find the target container with more flexible comparison
    target_container = None
    for container in containers.values():
        if str(container.id).strip().lower() == str(request.containerId).strip().lower():
            target_container = container
            break

    if not target_container:
        available = [c.id for c in containers.values()]
        logger.error(f"Container {request.containerId} not found. Available: {available}")
        raise HTTPException(status_code=404, detail=f"Container {request.containerId} not found. Available: {available}")
    
    # Check if item already exists in storage
    if item_id in storage_map:
        old_zone, old_pos = storage_map[item_id]
        old_container = next((c for c in containers.values() if c.zone == old_zone), None)

        if old_container:
            old_container.stored_items = [i for i in old_container.stored_items if i.id != item_id]

    # Locate existing item (or create new one)
    item_obj = next(
        (i for c in containers.values() for i in c.stored_items if i.id == item_id), None
    )

    if not item_obj:
        # Create new placeholder item if not found
        item_obj = Item(
            id=item_id,
            name=f"Item {item_id}",
    ...
    # Validate item fits inside container
    if (position.endCoordinates.width > target_container.width_cm or
        position.endCoordinates.depth > target_container.depth_cm or
        position.endCoordinates.height > target_container.height_cm):
        raise HTTPException(status_code=400, detail="Item does not fit in the container")

    # Check for overlapping items
    for existing_item in target_container.stored_items:
        if (position.startCoordinates.width < existing_item.width_cm and
            position.endCoordinates.width > 0 and
            position.startCoordinates.depth < existing_item.depth_cm and
            position.endCoordinates.depth > 0 and
            position.startCoordinates.height < existing_item.height_cm and
            position.endCoordinates.height > 0):
            raise HTTPException(status_code=400, detail="Position overlaps with an existing item")
...
async def import_containers(file: UploadFile = File(...)):
    ...
    try:
        ...
        container = Container(
            id=container_id.strip(),  # Ensure we strip whitespace
            zone=zone,
            width_cm=float(width),
            depth_cm=float(depth),
            height_cm=float(height),
        ...
        # Make sure this matches your storage structure
        containers[container.id] = container  # or whatever your storage structure is
